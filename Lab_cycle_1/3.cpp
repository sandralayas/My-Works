...
void Binary_search(int DATA[], int L, int U, int ITEM, int LOC){
	int START=L,STOP=U;
	int MID=(START+STOP)/2;
	LOC=-1;
	while(DATA[MID]!=ITEM and START<=STOP){
		if(ITEM<DATA[MID])STOP=MID-1;
		else START=MID+1;
		MID=MID=(START+STOP)/2;
	}
...
\n\n Enter your choice : ";cin>>choice;
	...
	if(choice=='0')break;
		...
		int L,U,LOC,ITEM;
		cout<<"\n\n Enter the item for binary search : ";cin>>ITEM;
		cout<<"\n\n Enter the lower bond of the binary search : ";cin>>L;
		cout<<"\n\n Enter the upper bond of the binary search : ";cin>>U;
		Binary_search(DATA,L,U,ITEM,LOC);
	}
	else cout<<"\n Wrong Choice!!\n";
	}while(choice!='0');
